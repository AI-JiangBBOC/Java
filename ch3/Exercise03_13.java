...
public class Exercise03_13 {
	public static void main(String[] args) {
		...
		System.out.println("(0-single filer,1-married jointly of" +
		"qualifying window(er),2-married separately,3-head of " +
				"household) Enter the filing status: ");
		int status = input.nextInt();//输入身份
		System.out.println("Enter he taxable income:");
		double income = input.nextDouble();//输入工资
		double tax = 0;//定义税金。
		if (status == 0) {
			if(income <= 8350)
				tax = income * 0.10;
			...
			else if(income <= 52_250)
				tax = 8350 * 0.10 + (33950 - 8350) * 0.15 + (income - 33950) * 0.25;
			else if(income <= 171_550)
				tax = 8350 * 0.10 + (33950 - 8350) * 0.15 + (82_250 - 33950) * 0.25 +
				(income - 82_250) * 0.28;
